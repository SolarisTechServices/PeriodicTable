...
import com.su.periodictable.MyMarkerView;
...
/*        values.add(new Entry(3, 80));
    ...
    public void getData(){
        ...
        switch(Trenddata.trendx){
            ...
            case 3:
                if(Trenddata.trendy==1){
                    size = Trenddata.graphmpgrp[Trenddata.trendz].length;
                    int[] graph = new int[size];
                    for (int i = 0; i < size; i++) {
                        graph[i] = (int) Trenddata.graphmpgrp[Trenddata.trendz][i];
                    }
                    graphy = graph;
                }else if(Trenddata.trendy==2){
                    size = Trenddata.graphmpprd[Trenddata.trendz].length;
                    int[] graph = new int[size];
                    for (int i = 0; i < size; i++) {
                        graph[i] = (int) Trenddata.graphmpprd[Trenddata.trendz][i];
                    }
                    graphy = graph;
                }
                break;
            case 4:
                if(Trenddata.trendy==1){
                    size = Trenddata.graphbpgrp[Trenddata.trendz].length;
                    int[] graph = new int[size];
                    for (int i = 0; i < size; i++) {
                        graph[i] = (int) Trenddata.graphbpgrp[Trenddata.trendz][i];
                    }
                    graphy = graph;
                }else if(Trenddata.trendy==2){
                    size = Trenddata.graphbpprd[Trenddata.trendz].length;
                    int[] graph = new int[size];
                    for (int i = 0; i < size; i++) {
                        graph[i] = (int) Trenddata.graphbpprd[Trenddata.trendz][i];
                    }
                    graphy = graph;
                }
                break;
